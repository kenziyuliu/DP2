...
class Trainer(BaseTrainer):
  """Non-private AdaGrad baseline."""
  def __init__(self, args):
    super(Trainer, self).__init__(args)

  def train(self):
    # NOTE: Use `jax.example_libraries.optimizers` for a reliable baseline.
    adagrad_opt = optimizers.adagrad(self.lr, momentum=0.0)
    ...
    def init_state(key):
      ...
      return adagrad_opt.init_fn(init_params)

    @jit
    def batch_update(iter_idx, key, state, batch):
      key = random.fold_in(key, iter_idx)
      params = adagrad_opt.params_fn(state)
      ...
      return batch_loss, batch_acc, key, adagrad_opt.update_fn(iter_idx, final_grad, state)
    ...
    for epoch_idx in range(self.epochs):
      if epoch_idx % self.eval_every_epoch == 0:
        self.evaluate(epoch_idx, params=adagrad_opt.params_fn(state))
