...
class Trainer(BaseTrainer):
  ...
  Same as DP-RMSProp, except
  - for every batch, we update the preconditioner using a fixed batch of randomly
    sampled training data as "public data" (full batch on these public data)
  - we apply preconditioning before adding noise

  Non private command gets to ~87% (similar to RMSProp):
  ```
  python3 main.py --dataset imdb --method dp_rmsprop_adadps \
    --clip1 1e10 --sigma 0.0 --lr 0.3 --epsilon 1e-5
  ```

  Private command gets to ~82% (similar to dp2_rmsprop):
  ```
  python3 main.py $IMDB_FLAGS --method dp_rmsprop_adadps \
    --lr 1.0 --clip1 5.0 --epsilon 1e-3
  ```

  Hyperparams may be different than reported in the AdaDPS paper since we don't
  use microbatches.
  """
  def __init__(self, args):
    ...
    print('[DP-RMSProp-AdaDPS] sampling training data as public data...')
  ...
  def train(self):
    ...
    def batch_update(iter_idx, key, state, batch):
      ...
      precond = struct_add(struct_mul_scalar(precond, self.rmsprop_gamma),
                           struct_mul_scalar(struct_sq(public_grad), 1.0 - self.rmsprop_gamma))
