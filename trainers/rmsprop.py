...
class Trainer(BaseTrainer):
  """Non-private RMSProp baseline."""
  def __init__(self, args):
    super(Trainer, self).__init__(args)

  def train(self):
    # NOTE: Use `jax.example_libraries.optimizers` for a reliable baseline.
    rmsprop_opt = optimizers.rmsprop(self.lr, gamma=self.rmsprop_gamma, eps=self.epsilon)
    ...
    def init_state(key):
      ...
      state = rmsprop_opt.init_fn(init_params)
      print('[INFO] Num parameters: ', jax_utils.num_params(init_params))
      return state

    @jit
    def batch_update(iter_idx, key, state, batch):
      key = random.fold_in(key, iter_idx)
      params = rmsprop_opt.params_fn(state)
      ...
      return batch_loss, batch_acc, key, rmsprop_opt.update_fn(iter_idx, final_grad, state)
    ...
    for epoch_idx in range(self.epochs):
      if epoch_idx % self.eval_every_epoch == 0:
        self.evaluate(epoch_idx, params=rmsprop_opt.params_fn(state))
